...
async function allTurnover(sales, expenses) {
  ...
  const byWorker = workers.map((w) => {
    ...
    const cashPaid = ws.filter((s) => s.payment_type === 'cash' && s.payment_status === 'paid').reduce((sum, s) => sum + s.total, 0);
    ...
    return {
      ...
      cash_paid: cashPaid,
      card_paid: ws.filter((s) => s.payment_type === 'card' && s.payment_status === 'paid').reduce((sum, s) => sum + s.total, 0),
      bank_paid: ws.filter((s) => s.payment_type === 'bank' && s.payment_status === 'paid').reduce((sum, s) => sum + s.total, 0),
  ...
  const total = {
    ...
    cash_paid: sales.filter((s) => s.payment_type === 'cash' && s.payment_status === 'paid').reduce((sum, s) => sum + s.total, 0),
    card_paid: sales.filter((s) => s.payment_type === 'card' && s.payment_status === 'paid').reduce((sum, s) => sum + s.total, 0),
    bank_paid: sales.filter((s) => s.payment_type === 'bank' && s.payment_status === 'paid').reduce((sum, s) => sum + s.total, 0),
    pending: sales.filter((s) => s.payment_status === 'pending').reduce((sum, s) => sum + s.total, 0),
    expenses_total: expenses.reduce((s, e) => s + e.amount, 0),
  };
